...
struct Hasher<Tuple, 0> {
    static void Hash(std::size_t& seed, const Tuple& tuple) { CombineHash(seed, std::get<0>(tuple)); }
};
...
struct Color {
    ...
    uint32_t ToInt() const {
        return ((static_cast<int>(R * 255) & 0xff) << 0) | ((static_cast<int>(G * 255) & 0xff) << 8) |
               ((static_cast<int>(B * 255) & 0xff) << 16) | ((static_cast<int>(A * 255) & 0xff) << 24);
    }

    void UseAsDrawColor(SDL_Renderer* renderer) const {
        SDL_SetRenderDrawColor(renderer, static_cast<uint8_t>(R * 255), static_cast<uint8_t>(G * 255),
...
struct Device {
    ...
    LRUCache<UniformColorTriangleKey, std::unique_ptr<TriangleCacheItem>, UniformColorTriangleCacheSize>
    ...
    SDL_Texture* MakeTexture(int width, int height) {
        SDL_Texture* texture =
            SDL_CreateTexture(Renderer, SDL_PIXELFORMAT_RGBA32, SDL_TEXTUREACCESS_TARGET, width, height);
...
class InterpolatedFactorEquation {
   public:
    InterpolatedFactorEquation(const T& value0,
                               const T& value1,
                               const T& value2,
                               const ImVec2& v0,
                               const ImVec2& v1,
...
struct Rect {
    ...
    static Rect CalculateBoundingBox(const ImDrawVert& v0, const ImDrawVert& v1, const ImDrawVert& v2) {
        return Rect{std::min({v0.pos.x, v1.pos.x, v2.pos.x}), std::min({v0.pos.y, v1.pos.y, v2.pos.y}),
                    std::max({v0.pos.x, v1.pos.x, v2.pos.x}), std::max({v0.pos.y, v1.pos.y, v2.pos.y}),
                    std::min({v0.uv.x, v1.uv.x, v2.uv.x}),    std::min({v0.uv.y, v1.uv.y, v2.uv.y}),
                    std::max({v0.uv.x, v1.uv.x, v2.uv.x}),    std::max({v0.uv.y, v1.uv.y, v2.uv.y})};
    }
};
...
struct FixedPointTriangleRenderInfo {
    ...
    static FixedPointTriangleRenderInfo CalculateFixedPointTriangleInfo(const ImVec2& v1,
                                                                        const ImVec2& v2,
                                                                        const ImVec2& v3) {
...
void DrawTriangleWithColorFunction(const FixedPointTriangleRenderInfo& renderInfo,
    ...
    // https://web.archive.org/web/20171128164608/http://forum.devmaster.net/t/advanced-rasterization/6145. This is a
    // fixed point implementation that rounds to top-left.
...
void DrawTriangle(const ImDrawVert& v1, const ImDrawVert& v2, const ImDrawVert& v3, const Texture* texture) {
    // The naming inconsistency in the parameters is intentional. The fixed point algorithm wants the vertices in a
    // counter clockwise order.
    const auto& renderInfo = FixedPointTriangleRenderInfo::CalculateFixedPointTriangleInfo(v3.pos, v2.pos, v1.pos);

    // First we check if there is a cached version of this triangle already waiting for us. If so, we can just do a
    // super fast texture copy.
...
void DrawUniformColorTriangle(const ImDrawVert& v1, const ImDrawVert& v2, const ImDrawVert& v3) {
    ...
    const auto key = std::make_tuple(v1.col, static_cast<int>(std::round(v1.pos.x)) - renderInfo.MinX,
...
void DrawRectangle(const Rect& bounding,
                   SDL_Texture* texture,
                   int textureWidth,
                   int textureHeight,
                   const Color& color,
                   bool doHorizontalFlip,
                   bool doVerticalFlip) {
    // We are safe to assume uniform color here, because the caller checks it and and uses the triangle renderer to
    // render those.

    const SDL_Rect destination = {static_cast<int>(bounding.MinX), static_cast<int>(bounding.MinY),
    ...
    if (bounding.UsesOnlyColor()) {
        ...
        const SDL_Rect source = {static_cast<int>(bounding.MinU * textureWidth),
...
void Render(ImDrawData* drawData) {
    ...
    for (int n = 0; n < drawData->CmdListsCount; n++) {
        ...
        for (int cmd_i = 0; cmd_i < commandList->CmdBuffer.Size; cmd_i++) {
            ...
            const Device::ClipRect clipRect = {static_cast<int>(drawCommand->ClipRect.x),
            ...
            if (drawCommand->UserCallback) {
                ...
                for (unsigned int i = 0; i + 3 <= drawCommand->ElemCount; i += 3) {
                    ...
                    // Actually, since we render a whole bunch of rectangles, we try to first detect those, and render
                    // them more efficiently. How are rectangles detected? It's actually pretty simple: If all 6
                    // vertices lie on the extremes of the bounding box, it's a rectangle.
                    if (i + 6 <= drawCommand->ElemCount) {
                        ...
                        if (isUniformColor && bounding.IsOnExtreme(v0.pos) && bounding.IsOnExtreme(v1.pos) &&
                            bounding.IsOnExtreme(v2.pos) && bounding.IsOnExtreme(v3.pos) &&
                            bounding.IsOnExtreme(v4.pos) && bounding.IsOnExtreme(v5.pos)) {
                            // ImGui gives the triangles in a nice order: the first vertex happens to be the topleft
                            // corner of our rectangle. We need to check for the orientation of the texture, as I
                            // believe in theory ImGui could feed us a flipped texture, so that the larger texture
                            // coordinates are at topleft instead of bottomright. We don't consider equal texture
                            // coordinates to require a flip, as then the rectangle is mostlikely simply a colored
                            // rectangle.
                    ...
                    if (isTriangleUniformColor && doesTriangleUseOnlyColor) {
                        ...
                        // Currently we assume that any non rectangular texture samples the font texture. Dunno if
                        // that's what actually happens, but it seems to work.
